    ...
    <div class="card-body">
        <form action="<?= $form["action"] ?>" method="POST" data-mode="<?= $mode ?>">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="tipo" id="tipo" class="form-control" value="<?= isset($tipoLicenza) ? esc($tipoLicenza['tipo']) : '' ?>" required>
            </div>
            <div class="mb-3 ">
                <label for="modello" class="form-label">Modello</label>
                <select name="modello" id="modello" class="form-select" required>
                    <option value="Common">-- Seleziona --</option>
                    <option value="Start" <?= (isset($tipoLicenza) && $tipoLicenza["modello"] === 'Start') ? 'selected' : '' ?>>Start</option>
                    <option value="Ultimate" <?= (isset($tipoLicenza) && $tipoLicenza["modello"] === 'Ultimate') ? 'selected' : '' ?>>Ultimate</option>
                    <option value="Cloud" <?= (isset($tipoLicenza) && $tipoLicenza["modello"] === 'Cloud') ? 'selected' : '' ?>>Cloud</option>
                    <option value="Unico" <?= (isset($tipoLicenza) && $tipoLicenza["modello"] === 'Unico') ? 'selected' : '' ?>>Unico</option>
                </select>
            </div>
            <div class="mb-3">
